    ...
    public function getAlbumById($id)
    {
        try {
            ...
            foreach ($json['webLinks'] as $link) {
                ...
                if (
                    !$link['disabled'] ||
                    !in_array($link['category'] ?? null, ['Commercial', 'Official'], true) ||
                    in_array($link['description'] ?? null, ['Website', 'Pixiv'], true)
                ) {
                    continue;
                }
            ...
            if ($e->response->status() === 404) {
                abort(404, 'No se ha encontrado el álbum');
            }

            abort(500, 'Error al obtener el álbum');
        }
    }
